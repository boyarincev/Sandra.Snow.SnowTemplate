...
- Чтобы сделать последний сегмент "жадным", так чтобы он поглощал всю оставшуюся строку адреса нужно перед его именем поставить '*': `{controller}/{action}/{*allRest}`  - будет соответствовать как адресу: "/home/index/2", так и адресу: "/home/index/2/4-52", в первом случае сегменту allRest будет соответствовать значение "2", а во втором значение "2/4-52".
...
Использование ограничений не изменилось. Строковое обозначение ограничений для использования в шаблоне можно посмотреть в классе [RouteOptions](https://github.com/aspnet/Routing/blob/master/src/Microsoft.AspNet.Routing/RouteOptions.cs).
...
В этом посте мы рассмотрели способы подключения системы маршрутизации к ASP.NET 5 приложению и ее настройку. А также изменения в использовании Attribute-Based маршрутизации и новые возможности шаблона маршрута.